...
- These scripts will only work on a `Raspberry Pi 4/5`, that is being used to display ScreenStage
...
```
## Troubleshooting
- Limited support for ScreenStage is available on the maintainer [Discord](https://discord.gg/AEhVjqX4Af) (original Posterr community: [Discord](https://discord.gg/TcnEkMEf9J)).
...
- The sleepwatch script polls the ScreenStage `/api/sleep` endpoint every 5 seconds to determine if ScreenStage is in scheduled sleep mode or not. It then issues an `on` or `standby` command to the attached display.

The script does the following:
- Gathers the URL for ScreenStage, current user and home directory
