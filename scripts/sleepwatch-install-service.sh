...
if [ $EUID -ne 0 ]; then
    echo
    echo "ScreenStage CEC display service install"
    echo "-----------------------------------"
    echo
    # gather parameters
    working_dir=$(pwd)
    user=$USER
    read -p "What is the full url for your ScreenStage instance? 
(eg http://192.168.1.134:9876) " url
    echo "ScreenStage URL: ${url}"
    echo "User name: ${user}"
    ...
    echo "Updated sleepwatch.sh script with ScreenStage url"
    echo
